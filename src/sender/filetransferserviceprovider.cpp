...
FileTransferServiceProvider::~FileTransferServiceProvider()
{
    ...
    if(this->fileSender!=nullptr)
        delete(this->fileSender);

    if(this->channelMetadata!=nullptr)
        delete(this->channelMetadata);

}

void FileTransferServiceProvider::run()
{
...
void FileTransferServiceProvider::runFileServiceProvider()
{
    ...
    if(filesystem::exists(this->filePath))
    {
        ...
        const unique_ptr<QLowEnergyController> btController(QLowEnergyController::createPeripheral(bleBootstrapChannel->getSelectedLocalBLEAdapter()));
...
QVector<VeniceMessage*> FileTransferServiceProvider::readFileData(const string& name, const int& max_size)
{
...
void FileTransferServiceProvider::configureDataChannel() throw()
{
    this->dataChannel->configure();
}

void FileTransferServiceProvider::configureBoostrapChannel() throw()
{
